...
void cb_create(device *device, _command_buffer *cb, size_t block_size)
{
  cb->submitting = false;
  cnd_init(&cb->cond);
...
void cb_destroy(device *device, _command_buffer *cb)
{
  cnd_destroy(&cb->cond);
  mtx_destroy(&cb->mtx);
...
void cb_begin(_command_buffer *cb)
{
...
void cb_end(_command_buffer *cb)
{
...
void cb_bind_vertex_buffer(_command_buffer *cb, buffer buffer, size_t binding)
{
  if (!cb->recording)
    return;
}

void cb_bind_index_buffer(_command_buffer *cb, buffer buffer)
{
  if (!cb->recording)
    return;
}

void cb_bind_pipeline(_command_buffer *cb, pipeline pipeline)
{
  if (!cb->recording)
    return;
}

void cb_draw(_command_buffer *cb, const draw_call_params *draw_call_params)
{
  if (!cb->recording)
    return;
}

void cb_reset(_command_buffer *cb)
{
  if (!cb->recording)
    return;
...
typedef struct
{
  ...
  vertex_attrib_gl attribs[TZ_MAX_ATTRIBUTES];
  size_t num_attribs;
  size_t stride;
} pipeline_gl;

TZ_POOL_VECTOR(gl_pipeline, pipeline_gl)
...
TZ_GFX_CREATE_DEVICE(create_device_gl3)
{
  create_gl_pipeline_pool(&g_device_gl.pipelines, device_config->resource_count.max_pipelines, device_config->allocator);
...
TZ_GFX_DELETE_DEVICE(delete_device_gl3)
{
  glDeleteVertexArrays(1, &g_device_gl.dummy_vao);
}

static GLuint compile_shader(const allocator *allocator, GLenum type, const shader_stage_params *shader_stage_create_info)
{
...
TZ_GFX_CREATE_SHADER(create_shader_gl3)
{
  shader shader_id = {TZ_GL_INVALID_ID};
  ...
  shader_id = (shader){program};

  return shader_id;
}

TZ_GFX_DELETE_SHADER(delete_shader_gl3)
{
  glDeleteProgram((GLuint)shader.data);
}

TZ_GFX_CREATE_BUFFER(create_buffer_gl3)
{
  buffer buffer_id = {TZ_GL_INVALID_ID};
...
TZ_GFX_DELETE_BUFFER(delete_buffer_gl3)
{
  GLuint handle = (GLuint)buffer.data;
  glDeleteBuffers(1, &handle);
}

TZ_GFX_CREATE_PIPELINE(create_pipeline_gl3)
{
  ...
  return (pipeline){gl_pipeline_pool_push(&g_device_gl.pipelines, new_pipeline)};
}

TZ_GFX_DELETE_PIPELINE(delete_pipeline_gl3)
{
  if (!gl_pipeline_pool_delete(&g_device_gl.pipelines, pipeline.data))
    TZ_LOG("Pipeline deletion error: invalid ID.\n");
}

static void gl3_clear_pipeline(pipeline pipeline_id)
{
...
static void gl3_bind_pipeline(draw_state *current_draw_state, const pipeline pipeline_id)
{
  pipeline_gl *pipeline_gl = gl_pipeline_pool_get_p(&g_device_gl.pipelines, pipeline_id.data);
...
static void gl3_bind_resources(pipeline pipeline_id, draw_state *current_draw_state, const draw_resources *draw_resources)
{
  pipeline_gl *pipeline_gl = gl_pipeline_pool_get_p(&g_device_gl.pipelines, pipeline_id.data);
  ...
  for (size_t attrib_index = 0; attrib_index < pipeline_gl->num_attribs; attrib_index++)
  {
    ...
    buffer buffer = draw_resources->buffers[attrib->buffer_binding];
...
TZ_GFX_EXECUTE_DRAW_GROUPS(execute_draw_groups_gl3)
{
  draw_state current_draw_state = {0};
  for (size_t i = 0; i < num_draw_groups; i++)
  {
    pipeline pipeline = draw_groups[i].pipeline;
    const draw_resources *resources = &draw_groups[i].resources;

    TZ_ASSERT(gl_pipeline_pool_index_is_valid(&g_device_gl.pipelines, pipeline.data),
...
device_params default_gfx_device_params()
{
  return (device_params){
      ...
      .allocator = default_cb_allocator(),
...
void init(device_params *device_config)
{
  ...
  case OPENGL3:
    ...
    invalid_handle = TZ_GL_INVALID_ID;
    break;
  }
